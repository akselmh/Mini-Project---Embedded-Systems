...
typedef ap_axiu<32,0,0,0> AxiStream32;
//int nn_inference(float input_img[n_inputs]){
void nn_inference(hls::stream< AxiStream32 > &INPUT, hls::stream< AxiStream32 > &OUTPUT, ap_uint<32> &res){
...
#pragma HLS PIPELINE II=1
    	AxiStream32 in_word = INPUT.read();

        // Reinterpret 32-bit stream data as float
        union { uint32_t i; float f; } conv;
        conv.i = (uint32_t)in_word.data;
        input_img[i] = conv.f;
	...
	res_reg = (ap_uint<32>)prediction;

    AxiStream32 out;
    out.data = (ap_uint<32>)prediction;
    out.keep = -1;
    out.strb = -1;
    out.user = 0;
    out.id   = 0;
